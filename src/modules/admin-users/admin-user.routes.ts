...
import { csrfProtection, requireAdmin, requireAuth, requireOwner } from '../auth/auth.middleware.js';
...
export function adminUserRoutes(authService: AuthService, service: AdminUserManagementService) {
  ...
  router.get('/', requireAuth(authService), requireOwner, asyncHandler(async (_request, response) => {
  ...
  router.patch('/:id/active', requireAuth(authService), requireOwner, csrfProtection(authService), asyncHandler(async (request, response) => {
